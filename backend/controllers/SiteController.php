...
class SiteController extends Controller
{
    ...
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => ['login', 'error','captcha','user-info','menu','message'],
                        'allow' => true,
                    ],
                    [
    ...
    public function actionLogin()
    {
        ...
        if ($model->load(Yii::$app->request->get()) && $model->login()) {
            return $this->success(['access_token'=>time()]);
        } else {
            return $this->error(-1,Json::encode($model->getErrors()));
        }
    }

    public function actionUserInfo(){
        if(!Yii::$app->user->isGuest)
            return $this->success(['username'=>Yii::$app->user->getIdentity()->username]);
        else
